...
LLM을 더 똑똑하게 만드는 도구는 아닙니다. 제공하시는 AI 기반
프로덕트의 **안전성**을 붙잡는 쪽에 주력했습니다. 소비자에게 닿는
AI가 예상 밖의 출력을 내서 사고로 번지지 않도록 하기 위함입니다 —
슬쩍 풀려버리는 거절, 흘러나오는 내부 지시, 정중하게 몇 번 다시
물어보면 부드러워지는 규칙 같은 것들입니다.

이 레포가 역해석한 Opus 4.7 원문도 똑같은 관점에서 쓰인 문서입니다.
그 1,408줄은 대부분 capability 튜닝이 아니라 guardrail입니다.
opus-mind는 그 프레이밍을 그대로 물려받았습니다. LINT는 Anthropic이
원문에서 막으려 했던 실패 모드에 대해 프로덕션 시스템 프롬프트를
강화하는 쪽입니다.

(capability 튜닝 — CoT, 툴 사용, 출력 형식 — 을 하고 싶으시다면 BOOST
쪽을 쓰시면 됩니다. 사용자 프롬프트 담당이고 전혀 다른 일입니다.)
