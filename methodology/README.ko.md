...
*증거:* the source lines 640–641 (15단어 하드 리밋), 620 (툴 콜 스케일링), 871–872 (이미지 min 3 max 4).

### I2. 규칙 충돌을 제거하라

규칙 충돌은 시스템 프롬프트의 사일런트 킬러. 공존 가능해 보이는 두 규칙이 어떤 입력에서 모순되고, 모델은 바이브로 한 쪽을 고른다. 너는 그 일이 일어나는지도 못 보고 — 그냥 일관성 없는 행동만 받는다.

**Opus 4.7의 수:** **first-match-wins 결정 래더**. `{request_evaluation_checklist}` 섹션 (원문 line 515) 이 모델을 Step 0 → Step 3으로 걷게 하고, 첫 매치에서 멈춘다. 규칙 충돌 불가능 — 래더 자체가 해결 순서.
